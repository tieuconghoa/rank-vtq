    ...
    oldData.forEach((itemOld, keyOld) => {
      $("#table").append(
        `<tr><td class='text-center'>${
          keyOld + 1
        }</td><td class='text-center'><div class='name-character' onclick="copyName(this)">${
          itemOld.name
        }</div></td><td class='text-center'>${
          itemOld.areaName
        }</td><td class='text-center chenh-lenh'>${String(
          itemOld.value
        ).replace(/(\d)(?=(\d\d\d)+(?!\d))/g, "$1,")}</td></tr>`
      );
    });
  });
}
    ...
    $.ajax({
      ...
      success: function (oldData) {
        ...
        if (oldData == "[]") {
          ...
          oldData.forEach((itemOld, keyOld) => {
            $("#table").append(
              `<tr class="item"><td class='text-center'>${
                keyOld + 1
              }</td><td class='text-center'><div class='name-character' onclick="copyName(this)">${
                itemOld.name
              }</div></td><td class='text-center'>${
                itemOld.areaName
              }</td><td class='text-center'>${String(itemOld.value).replace(
                /(\d)(?=(\d\d\d)+(?!\d))/g,
                "$1,"
              )}</td></tr>`
            );
